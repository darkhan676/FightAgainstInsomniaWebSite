...
<body>
    ...
    if (isset($_POST['username']) && isset($_POST['email']) && isset($_POST['password'])) {
        ...
        if ($password_confirm === $password) {
            $path = 'avatarka/' . md5(uniqid()) . $_FILES['profile_picture']['name'];
            if (!move_uploaded_file($_FILES['profile_picture']['tmp_name'], $path)) {
                $path_null = 'avatarka_standard/avatar.png';
                mysqli_query($connection, "INSERT INTO users(fullname, username, email, phone_number, password, profile_picture) 
                                           VALUES('$fullname', '$username', '$email', '$phone_number', '$password', '$path_null')");
            }

            mysqli_query($connection, "INSERT INTO users(fullname, username, email, phone_number, password, profile_picture) 
                                           VALUES('$fullname', '$username', '$email', '$phone_number', '$password', '$path')");
